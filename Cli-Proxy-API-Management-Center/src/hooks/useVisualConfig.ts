// 本文件实现 config.yaml 与可视化配置表单之间的双向转换。
// 具体内容：
// 1. 读取 YAML 并转换为 VisualConfigValues，供配置面板直接编辑。
// 2. 追踪每个字段是否被修改，并只把改动字段写回 YAML。
// 3. 维护 api-keys 与 api-key-names 的联动保存，让每个客户端 Key 可以绑定使用人名字。
...
function apiKeysTextToKeySet(apiKeysText: string): Set<string> {
  return new Set(
    apiKeysText
      .split('\n')
      .map((key) => key.trim())
...
function normalizeApiKeyNamesForApiKeysText(
  apiKeysText: string,
  names: Record<string, string>
): Record<string, string> {
  const allowedKeys = apiKeysTextToKeySet(apiKeysText);
...
function parseApiKeyNames(raw: unknown): Record<string, string> {
  const record = asRecord(raw);
  if (!record) return {};
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (!trimmedKey || !trimmedName) continue;
...
function setStringRecordInDoc(
  ...
  for (const [key, value] of Object.entries(values)) {
    ...
    nextValues[trimmedKey] = trimmedValue;
  }
...
function areStringRecordsEqual(
  ...
  const rightKeys = Object.keys(rightItems).sort();
  if (!areStringArraysEqual(leftKeys, rightKeys)) return false;
  return leftKeys.every((key) => leftItems[key] === rightItems[key]);
}
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const newValues: VisualConfigValues = {
        ...
        apiKeysText,
        apiKeyNames: normalizeApiKeyNamesForApiKeysText(
          apiKeysText,
          parseApiKeyNames(parsed['api-key-names'])
        ),
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        if (dirtyFields.has('apiKeysText') || dirtyFields.has('apiKeyNames')) {
          setStringRecordInDoc(
            doc,
            ['api-key-names'],
            normalizeApiKeyNamesForApiKeysText(values.apiKeysText, values.apiKeyNames)
          );
        }
