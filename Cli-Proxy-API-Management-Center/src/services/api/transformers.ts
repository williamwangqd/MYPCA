// 本文件实现后端配置响应到前端结构的转换。
// 具体内容：
// 1. 将后端 YAML/JSON 字段转换为管理中心使用的 camelCase 配置对象。
// 2. 规范化各类模型、请求头、排除模型和 OAuth 配置。
// 3. 解析 api-key-names，保留每个客户端 API Key 对应的使用人名字。
import type {
  ApiKeyEntry,
  CloakConfig,
  GeminiKeyConfig,
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  config.apiKeyNames = normalizeStringRecord(raw['api-key-names']);
