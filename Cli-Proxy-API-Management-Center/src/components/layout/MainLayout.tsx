/**
 * 本文件实现管理中心的主布局结构，包括顶部工具栏、侧边导航、插件页面入口和主内容路由容器。
 * 侧边导航按照运行、网关、观测和控制等分组组织功能入口，并根据登录状态与插件资源动态刷新。
 * “快速开始”属于可直接访问的独立页面，但不再加入侧边菜单，避免占用常用功能的导航位置。
 * “用户使用信息统计”位于观测分组，提供按使用人查看汇总与逐次请求明细的入口。
 * 侧边栏底部固定显示 MYPCA 前端版本号，折叠时自动切换为紧凑展示。
 */
import {
  ReactNode,
  RefObject,
...
export function MainLayout() {
  ...
  const fullBrandName = 'MYPCA';
  const abbrBrandName = t('title.abbr');
  const rawAppVersion = __APP_VERSION__.replace(/^['"]+|['"]+$/g, '').trim() || '1.0.0';
  ...
  const navGroups: SidebarNavGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          path: '/user-usage',
          labelKey: 'nav.user_usage',
          metaKey: 'nav_meta.user_usage',
          icon: sidebarIcons.userUsage,
        },
        {
          path: '/logs',
          labelKey: 'nav.logs',
      ...
      <div className="main-body">
        ...
        >
          <div className="sidebar-brand" title={fullBrandName}>
            <img src={mypcaLogo} alt="MYPCA logo" className="sidebar-brand-logo" />
          ...
          <div className="sidebar-version" title={`${fullBrandName} ${appVersion}`}>
            {showSidebarLabels ? (
              <>
                <span className="sidebar-version-marker" aria-hidden="true" />
                <span className="sidebar-version-copy">
                  <span className="sidebar-version-product">{fullBrandName}</span>
                  <span className="sidebar-version-number">{appVersion}</span>
                </span>
              </>
            ) : (
              <span className="sidebar-version-compact">{appVersion}</span>
            )}
          </div>
        </aside>
