// 本文件实现可视化配置面板中的复杂编辑控件。
// 具体内容：
// 1. 提供 API Keys 卡片、字符串列表、payload 规则、插件认证规则等可复用编辑器。
// 2. API Keys 卡片支持为每个客户端 Key 维护使用人名字，并同步写入 api-key-names。
// 3. 各编辑器负责本地输入校验、弹窗编辑和结构化数据回传，避免调用方处理表单细节。
...
function normalizeApiKeyOwnerNames(
  apiKeys: string[],
  ownerNames: Record<string, string>
): Record<string, string> {
  const allowedKeys = new Set(apiKeys.map((key) => key.trim()).filter(Boolean));
  if (allowedKeys.size === 0) return {};

  const nextNames: Record<string, string> = {};
  for (const [key, name] of Object.entries(ownerNames)) {
    const trimmedKey = key.trim();
    const trimmedName = name.trim();
    if (!trimmedKey || !trimmedName || !allowedKeys.has(trimmedKey)) continue;
    nextNames[trimmedKey] = trimmedName;
  }
  return nextNames;
}
...
export const ApiKeysCardEditor = memo(function ApiKeysCardEditor({
  ...
  ownerNames?: Record<string, string>;
  disabled?: boolean;
  onChange: (nextValue: string, nextOwnerNames: Record<string, string>) => void;
}) {
  const { t } = useTranslation();
  ...
  const normalizedOwnerNames = useMemo(
    () => normalizeApiKeyOwnerNames(apiKeys, ownerNames),
    [apiKeys, ownerNames]
  );
  ...
  const openEditModal = (apiKeyId: string) => {
    ...
    const editingKey = apiKeys[editingIndex]?.trim() ?? '';
    setEditingApiKeyId(apiKeyId);
    setInputValue(apiKeys[editingIndex] ?? '');
    setOwnerNameValue(normalizedOwnerNames[editingKey] ?? '');
    setFormError('');
    setModalOpen(true);
  };
  ...
  const updateApiKeys = (nextKeys: string[], nextOwnerNames: Record<string, string>) => {
    onChange(nextKeys.join('\n'), normalizeApiKeyOwnerNames(nextKeys, nextOwnerNames));
  };

  const handleDelete = (apiKeyId: string) => {
    const index = renderApiKeyIds.findIndex((id) => id === apiKeyId);
    if (index < 0) return;
    const removedKey = apiKeys[index]?.trim() ?? '';
    const nextOwnerNames = { ...normalizedOwnerNames };
    if (removedKey) delete nextOwnerNames[removedKey];
  ...
  const handleSave = () => {
    ...
    const editingIndex = editingApiKeyId
      ...
      : -1;
    const previousKey = editingIndex >= 0 ? (apiKeys[editingIndex]?.trim() ?? '') : '';
    const nextKeys =
      editingApiKeyId === null
        ? [...apiKeys, trimmed]
        : apiKeys.map((key, idx) => (idx === editingIndex ? trimmed : key));
    const nextOwnerNames = { ...normalizedOwnerNames };
    if (previousKey && previousKey !== trimmed) delete nextOwnerNames[previousKey];
    const trimmedOwnerName = ownerNameValue.trim();
    if (trimmedOwnerName) {
      nextOwnerNames[trimmed] = trimmedOwnerName;
    } else {
      delete nextOwnerNames[trimmed];
    }
    if (editingApiKeyId === null) {
      setApiKeyIds([...renderApiKeyIds, makeClientId()]);
    }
    updateApiKeys(nextKeys, nextOwnerNames);
    closeModal();
  };
  ...
  return (
    <div className="form-group" style={{ marginBottom: 0 }}>
      ...
      {apiKeys.length === 0 ? (
        ...
        <div className="item-list" style={{ marginTop: 4 }}>
          {apiKeys.map((key, index) => {
            const ownerName = normalizedOwnerNames[key.trim()] ?? '';
            return (
              <div key={renderApiKeyIds[index] ?? `${key}-${index}`} className="item-row">
                <div className="item-meta">
                  ...
                  <div className="item-subtitle">
                    {ownerName
                      ? `${t('config_management.visual.api_keys.owner_label')}: ${ownerName}`
                      : t('config_management.visual.api_keys.owner_empty')}
                  </div>
                </div>
      ...
      >
        ...
        <div className="form-group">
          <label htmlFor={ownerNameInputId}>
            {t('config_management.visual.api_keys.owner_label')}
          </label>
          <input
            id={ownerNameInputId}
            className="input"
            placeholder={t('config_management.visual.api_keys.owner_placeholder')}
            value={ownerNameValue}
            onChange={(e) => setOwnerNameValue(e.target.value)}
            disabled={disabled}
          />
          <div className="hint">{t('config_management.visual.api_keys.owner_hint')}</div>
        </div>
      </Modal>
    </div>
  );
