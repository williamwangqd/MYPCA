// 本文件实现 config.yaml 的可视化编辑页面。
// 具体内容：
// 1. 组织简单模式与完整模式下的配置分区、搜索跳转和字段校验展示。
// 2. 将 api-keys 与 api-key-names 一起传给 API Keys 卡片，实现 Key 与使用人名字联动编辑。
// 3. 将用户表单改动转换为 VisualConfigValues 的局部更新，交给 useVisualConfig 写回 YAML。
import {
  useCallback,
  useEffect,
  useId,
...
export function VisualConfigEditor({
  ...
  const handleApiKeysTextChange = useCallback(
    (apiKeysText: string, apiKeyNames: Record<string, string>) =>
      onChange({ apiKeysText, apiKeyNames }),
    [onChange]
  );
