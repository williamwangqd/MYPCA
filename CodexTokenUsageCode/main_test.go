// File overview:
// These tests validate usage persistence, quota and credential auto-bans,
// management diagnostics, dashboard behavior, and scheduler integration. The
// scheduler coverage includes shared round-robin distribution after banned
// credentials are filtered so different API keys and models cannot collapse
// onto the first remaining account.
package main
...
func TestCodex429AutobanRoundRobinsRemainingHighestPriorityCandidates(t *testing.T) {
	t.Setenv("CPA_TOKEN_USAGE_DIR", t.TempDir())
	ctx := context.Background()
	store := &store{}
	defer store.close()
	...
	candidates := []schedulerAuthCandidate{
		{ID: "auth-banned", Provider: "codex", Priority: 100},
		{ID: "auth-a", Provider: "codex", Priority: 50},
		{ID: "auth-b", Provider: "codex", Priority: 50},
		{ID: "auth-c", Provider: "codex", Priority: 50},
		{ID: "auth-lower-priority", Provider: "codex", Priority: 10},
	}
	want := []string{"auth-a", "auth-b", "auth-c", "auth-a", "auth-b", "auth-c"}
	for index, wantAuthID := range want {
		resp, errPick := store.pickAuth(ctx, schedulerPickRequest{
			Provider: "codex",
			Model:    fmt.Sprintf("gpt-test-%d", index%2),
			Options: schedulerOptions{Metadata: map[string]any{
				"api_key": fmt.Sprintf("downstream-key-%d", index%3),
			}},
			Candidates: candidates,
		})
		if errPick != nil {
			t.Fatalf("pickAuth call %d returned error: %v", index, errPick)
		}
		if !resp.Handled || resp.AuthID != wantAuthID {
			t.Fatalf("pickAuth call %d = %+v, want handled %s", index, resp, wantAuthID)
		}
	}
}
