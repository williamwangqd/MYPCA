// 本文件实现 CLI Proxy API 服务端的基础 SDK 配置结构。
// 具体内容：
// 1. 定义服务端端口、代理、日志、客户端 API Key、流式输出等顶层 YAML 配置字段。
// 2. 为管理面板提供 api-key-names 映射，用来给每个下游客户端 Key 绑定使用人名字。
// 3. 保持 Go 结构体字段与 config.yaml / 管理接口 JSON 字段一一对应，方便配置加载、保存和前端展示。
...
type SDKConfig struct {
	...
	// APIKeyNames maps each downstream client API key to the person who should use it.
