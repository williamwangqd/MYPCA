// 本测试文件验证配置运行时克隆逻辑。
// 具体内容：
// 1. 确认 CloneForRuntime 会深拷贝切片、map、嵌套结构和 YAML 节点。
// 2. 覆盖 api-keys 与 api-key-names，避免客户端 Key 使用人映射在运行时快照中共享引用。
// 3. 验证克隆对象和原始配置互相修改时不会影响对方。
package config
...
func TestCloneForRuntimeDeepCopiesConfig(t *testing.T) {
	...
	if clone.APIKeyNames["client-key"] != "张三" {
		t.Fatalf("clone.APIKeyNames[client-key] = %q, want 张三", clone.APIKeyNames["client-key"])
	}
	...
	clone.APIKeyNames["client-key"] = "李四"
	...
	if cfg.APIKeyNames["client-key"] != "王五" {
		t.Fatalf("cfg.APIKeyNames[client-key] = %q, want 王五", cfg.APIKeyNames["client-key"])
	}
...
func sampleCloneRuntimeConfig() *Config {
	...
	return &Config{
		SDKConfig: SDKConfig{
			...
			APIKeyNames: map[string]string{"client-key": "张三"},
