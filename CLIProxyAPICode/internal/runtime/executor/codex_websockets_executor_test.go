...
func TestCodexWebsocketsEnsureUpstreamConnSwitchesWhenAuthChanges(t *testing.T) {
	...
	sessionID := "test-auth-change-reconnect"
	...
	conn1, _, err := exec.ensureUpstreamConn(context.Background(), nil, sess, "auth-a", wsURL, nil)
	if err != nil {
	...
	conn2, _, err := exec.ensureUpstreamConn(context.Background(), nil, sess, "auth-b", wsURL, nil)
	if err != nil {
	...
	if conn2 == conn1 {
		t.Fatal("auth change reused the previous upstream websocket connection")
	}
	select {
	case <-accepted:
	case <-time.After(5 * time.Second):
	...
	sess.connMu.Unlock()
	if gotAuthID != "auth-b" {
		t.Fatalf("session authID = %q, want auth-b", gotAuthID)
	}
}
