...
func (e *CodexWebsocketsExecutor) ensureUpstreamConn(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
	...
	if conn != nil && (currentAuthID != requestedAuthID || currentWSURL != requestedWSURL) {
		sess.conn = nil
	...
	if conn != nil {
		if currentAuthID != requestedAuthID || currentWSURL != requestedWSURL {
			logCodexWebsocketDisconnected(sessionID, currentAuthID, currentWSURL, "auth_changed", nil)
			if errClose := conn.Close(); errClose != nil {
				log.Errorf("codex websockets executor: close websocket error: %v", errClose)
			}
			conn = nil
