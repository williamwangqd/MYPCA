...
// 本文件的路由注册还包含客户端 Key 汇总与逐次交互明细接口，供管理中心按使用人查看统计和请求历史。
package api
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	// 捕获模型请求的原始提示词和最终客户端响应，供“用户使用统计”按人、按日持久化明细。
	// 中间件只建立内存捕获对象；只有成功产生 usage 记录的客户端 Key 请求才会真正写入磁盘。
	engine.Use(middleware.ClientUsageContentCaptureMiddleware())
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/client-key-usage/conversation-saving", s.mgmt.GetClientKeyUsageConversationSaving)
		mgmt.PUT("/client-key-usage/conversation-saving", s.mgmt.PutClientKeyUsageConversationSaving)
		mgmt.PATCH("/client-key-usage/conversation-saving", s.mgmt.PutClientKeyUsageConversationSaving)
		mgmt.POST("/client-key-usage/open-folder", s.mgmt.OpenClientKeyUsageFolder)
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	// 管理面板是单文件前端，发布后经常会被本地浏览器缓存。
	// 禁用缓存可以避免 /management.html#/ 这类 hash 路由继续命中旧页面。
