// 本文件实现管理接口中各类配置列表的增删改查。
// 具体内容：
// 1. 提供 api-keys、Gemini、Claude、Codex、OpenAI 兼容等配置列表的管理接口。
// 2. 在 api-keys 管理中维护 api-key-names，让每个下游客户端 Key 可绑定使用人名字。
// 3. 保存配置时复用管理处理器的持久化流程，确保 config.yaml 与内存配置保持一致。
package management
...
func (h *Handler) GetAPIKeys(c *gin.Context) {
	h.mu.Lock()
	apiKeys := []string(nil)
	apiKeyNames := map[string]string(nil)
	if h.cfg != nil {
		apiKeys = append(apiKeys, h.cfg.APIKeys...)
		apiKeyNames = normalizeAPIKeyNamesForKeys(apiKeys, h.cfg.APIKeyNames)
	}
	h.mu.Unlock()
	c.JSON(200, gin.H{"api-keys": apiKeys, "api-key-names": apiKeyNames})
}
...
func (h *Handler) PutAPIKeys(c *gin.Context) {
	...
	h.mu.Lock()
	defer h.mu.Unlock()
	h.cfg.APIKeys = append([]string(nil), arr...)
	h.cfg.APIKeyNames = normalizeAPIKeyNamesForKeys(h.cfg.APIKeys, h.cfg.APIKeyNames)
	h.persistLocked(c)
}
...
func (h *Handler) PatchAPIKeys(c *gin.Context) {
	...
	if body.Index != nil && body.Value != nil && *body.Index >= 0 && *body.Index < len(h.cfg.APIKeys) {
		oldKey := h.cfg.APIKeys[*body.Index]
		h.cfg.APIKeys[*body.Index] = *body.Value
		h.cfg.APIKeyNames = migrateAPIKeyName(h.cfg.APIKeyNames, oldKey, *body.Value)
		h.cfg.APIKeyNames = normalizeAPIKeyNamesForKeys(h.cfg.APIKeys, h.cfg.APIKeyNames)
		h.persistLocked(c)
		return
	}
	if body.Old != nil && body.New != nil {
		for i := range h.cfg.APIKeys {
			if h.cfg.APIKeys[i] == *body.Old {
				...
				h.cfg.APIKeyNames = migrateAPIKeyName(h.cfg.APIKeyNames, *body.Old, *body.New)
				h.cfg.APIKeyNames = normalizeAPIKeyNamesForKeys(h.cfg.APIKeys, h.cfg.APIKeyNames)
				h.persistLocked(c)
				return
			}
...
func (h *Handler) DeleteAPIKeys(c *gin.Context) {
	...
	if idxStr := c.Query("index"); idxStr != "" {
		...
		if err == nil && idx >= 0 && idx < len(h.cfg.APIKeys) {
			removedKey := h.cfg.APIKeys[idx]
			h.cfg.APIKeys = append(h.cfg.APIKeys[:idx], h.cfg.APIKeys[idx+1:]...)
			h.cfg.APIKeyNames = removeAPIKeyName(h.cfg.APIKeyNames, removedKey)
			h.cfg.APIKeyNames = normalizeAPIKeyNamesForKeys(h.cfg.APIKeys, h.cfg.APIKeyNames)
			h.persistLocked(c)
			return
		}
	...
	if val := strings.TrimSpace(c.Query("value")); val != "" {
		...
		for _, key := range removed {
			h.cfg.APIKeyNames = removeAPIKeyName(h.cfg.APIKeyNames, key)
		}
		h.cfg.APIKeyNames = normalizeAPIKeyNamesForKeys(h.cfg.APIKeys, h.cfg.APIKeyNames)
		h.persistLocked(c)
		return
...
func normalizeAPIKeyNamesForKeys(keys []string, names map[string]string) map[string]string {
...
func migrateAPIKeyName(names map[string]string, oldKey string, newKey string) map[string]string {
	oldKey = strings.TrimSpace(oldKey)
	newKey = strings.TrimSpace(newKey)
	if oldKey == "" || newKey == "" || oldKey == newKey || len(names) == 0 {
		return names
	}
	name := strings.TrimSpace(names[oldKey])
	delete(names, oldKey)
	if name != "" {
		if names == nil {
...
func removeAPIKeyName(names map[string]string, key string) map[string]string {
