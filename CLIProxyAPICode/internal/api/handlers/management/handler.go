...
// 本文件还负责：
// 1. 初始化管理处理器并配置客户端 Key 用量统计状态文件路径。
// 2. 提供跨平台本地文件夹打开器，使“用户使用信息统计”页面可以直接打开使用人的日志目录。
// 3. 允许测试替换文件夹打开函数，避免自动化测试真实启动资源管理器。
package management
...
func NewHandler(cfg *config.Config, configFilePath string, manager *coreauth.Manager) *Handler {
	...
	var configuredKeyNames map[string]string
	if cfg != nil {
		configuredKeys = cfg.APIKeys
		configuredKeyNames = cfg.APIKeyNames
	}
	configureClientKeyUsageState(configFilePath, configuredKeys, configuredKeyNames)
...
// openLocalFolder 使用当前操作系统的标准文件管理器打开目录。
// 该函数只负责启动系统程序，不等待文件管理器退出，避免管理接口被桌面窗口生命周期阻塞。
func openLocalFolder(path string) error {
	absolutePath, errAbsolute := filepath.Abs(strings.TrimSpace(path))
	if errAbsolute != nil {
		return fmt.Errorf("resolve folder path: %w", errAbsolute)
	}
	...
	case "darwin":
		command = exec.Command("open", absolutePath)
	default:
		command = exec.Command("xdg-open", absolutePath)
	}
	if errStart := command.Start(); errStart != nil {
		return fmt.Errorf("start folder opener: %w", errStart)
	}
	return nil
}
...
func (h *Handler) SetConfig(cfg *config.Config) {
	...
	h.mu.Unlock()
	if cfg != nil {
		globalClientKeyUsageCollector.SetConfiguredKeys(cfg.APIKeys, cfg.APIKeyNames)
