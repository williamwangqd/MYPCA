/*
本文件实现上游凭据选择策略，主要包含以下逻辑：
1. 普通请求继续使用按 provider 和模型隔离的轮询游标，保证兼容原有 round-robin 行为；
2. 所有下游 API Key 和模型共用 provider 级轮询游标，避免不同模型分别从同一个账号起步；
3. 同一个 Key 的连续请求也会继续轮换账号，按真实请求量均衡消耗全部可用账号；
4. 被禁用、限流或不支持目标模型的账号会先被过滤，轮询自动在剩余账号中继续。
*/
package auth
...
func (s *RoundRobinSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	// 所有下游 Key 共用 provider 级游标，并且不再按模型拆分游标。
	// 旧实现为每个 Key、每个模型分别维护进度，多个终端很容易各自在不同游标中
	// 选中相同账号。共享游标会让任意 Key、任意模型到达的新请求都取得下一个账号，
	// 因此同一个 Key 不会固定绑定账号，跨模型流量也能按照真实请求次数统一均衡。
	key := provider + ":" + canonicalModelKey(model)
	if hasDownstreamAPIKeyRoutingMetadata(opts.Metadata) {
		key = provider + ":downstream"
	}
	s.mu.Lock()
	if s.cursors == nil {
...
func downstreamAPIKeyRoutingIdentity(meta map[string]any) (string, int, bool) {
	if len(meta) == 0 {
		return "", 0, false
	}
	if index := stringMetadataValue(meta, cliproxyexecutor.DownstreamAPIKeyIndexMetadataKey); index != "" {
		offset := nonNegativeIntFromString(index)
		return "idx:" + index, offset, true
	}
	if keyHash := stringMetadataValue(meta, cliproxyexecutor.DownstreamAPIKeyHashMetadataKey); keyHash != "" {
		return "hash:" + keyHash, stableOffsetFromString(keyHash), true
	}
	return "", 0, false
}

func hasDownstreamAPIKeyRoutingMetadata(meta map[string]any) bool {
	_, _, ok := downstreamAPIKeyRoutingIdentity(meta)
	return ok
}

func nonNegativeIntFromString(raw string) int {
	parsed, err := strconv.Atoi(strings.TrimSpace(raw))
	if err != nil || parsed < 0 {
		return stableOffsetFromString(raw)
	}
	return parsed
}

func stableOffsetFromString(value string) int {
...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if clientIdentity, _, ok := downstreamAPIKeyRoutingIdentity(opts.Metadata); ok {
		primaryID = clientIdentity + "::" + primaryID
		if fallbackID != "" {
			fallbackID = clientIdentity + "::" + fallbackID
		}
	}
