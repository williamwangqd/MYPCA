/*
本文件验证凭据选择器的轮询、下游 Key 全局均衡、故障转移和会话亲和行为。
其中下游 API Key 用例重点保证不同 Key、不同模型共用轮询进度，同一个 Key 也不会固定绑定账号。
*/
package auth
...
func TestRoundRobinSelectorPick_DownstreamAPIKeysShareProviderCursor(t *testing.T) {
	t.Parallel()
	...
	optsKey0 := cliproxyexecutor.Options{Metadata: map[string]any{cliproxyexecutor.DownstreamAPIKeyIndexMetadataKey: "0"}}
	optsKey1 := cliproxyexecutor.Options{Metadata: map[string]any{cliproxyexecutor.DownstreamAPIKeyIndexMetadataKey: "1"}}
	optsKey2 := cliproxyexecutor.Options{Metadata: map[string]any{cliproxyexecutor.DownstreamAPIKeyIndexMetadataKey: "2"}}

	firstKey0, err := selector.Pick(context.Background(), "codex", "gpt-5.5", optsKey0, auths)
	if err != nil {
		t.Fatalf("Pick() key0 error = %v", err)
	}
	firstKey1, err := selector.Pick(context.Background(), "codex", "gpt-5.6-sol", optsKey1, auths)
	if err != nil {
		t.Fatalf("Pick() key1 error = %v", err)
	}
	secondKey0, err := selector.Pick(context.Background(), "codex", "gpt-5.4-mini", optsKey0, auths)
	if err != nil {
		t.Fatalf("Pick() key0 second error = %v", err)
	}
	firstKey2, err := selector.Pick(context.Background(), "codex", "gpt-5.5", optsKey2, auths)
	if err != nil {
		t.Fatalf("Pick() key2 error = %v", err)
	}

	if firstKey0.ID != "a" || firstKey1.ID != "b" || secondKey0.ID != "c" || firstKey2.ID != "a" {
		t.Fatalf(
			"shared downstream picks = %q, %q, %q, %q; want a, b, c, a",
			firstKey0.ID,
			firstKey1.ID,
			secondKey0.ID,
			firstKey2.ID,
	...
	selector.mu.Lock()
	cursorCount := len(selector.cursors)
	_, hasSharedCursor := selector.cursors["codex:downstream"]
	selector.mu.Unlock()
	if cursorCount != 1 || !hasSharedCursor {
		t.Fatalf("downstream cursors = %#v, want only %q", selector.cursors, "codex:downstream")
	}
}

func TestRoundRobinSelectorPick_DownstreamAPIKeySkipsUnavailableAuth(t *testing.T) {
	t.Parallel()
	...
	opts := cliproxyexecutor.Options{Metadata: map[string]any{cliproxyexecutor.DownstreamAPIKeyIndexMetadataKey: "2"}}

	selected, err := selector.Pick(context.Background(), "codex", "gpt-5.5", opts, auths)
	if err != nil {
		t.Fatalf("Pick() initial error = %v", err)
	}
	if selected.ID != "a" {
		t.Fatalf("initial auth.ID = %q, want %q", selected.ID, "a")
	}

	// 下一个轮询账号不可用时，应直接跳过它并继续使用剩余账号池。
	...
	if selected.ID != "c" {
		t.Fatalf("failover auth.ID = %q, want %q", selected.ID, "c")
	}
}
