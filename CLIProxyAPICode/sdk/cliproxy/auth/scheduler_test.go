/*
本文件验证调度器快速路径、传统选择器路径、优先级、模型可用性和下游 Key 全局均衡行为。
下游 API Key 用例用于确保调度器不会绕过 provider 级共享轮询逻辑。
*/
package auth
...
func TestManagerPickNext_DownstreamAPIKeysUseSharedSelectorCursor(t *testing.T) {
	t.Parallel()
	...
	gotKey1, _, errPick := manager.pickNext(context.Background(), "codex", "", optsKey1, nil)
	if errPick != nil {
		t.Fatalf("pickNext() key1 error = %v", errPick)
	}
	gotKey2, _, errPick := manager.pickNext(context.Background(), "codex", "", optsKey2, nil)
	if errPick != nil {
		t.Fatalf("pickNext() key2 error = %v", errPick)
	}
	gotKey1Again, _, errPick := manager.pickNext(context.Background(), "codex", "", optsKey1, nil)
	if errPick != nil {
		t.Fatalf("pickNext() key1 second error = %v", errPick)
	}

	if gotKey1.ID != "auth-a" {
		t.Fatalf("pickNext() key1 auth.ID = %q, want %q", gotKey1.ID, "auth-a")
	}
	if gotKey2.ID != "auth-b" {
		t.Fatalf("pickNext() key2 auth.ID = %q, want %q", gotKey2.ID, "auth-b")
	}
	if gotKey1Again.ID != "auth-c" {
		t.Fatalf("pickNext() key1 second auth.ID = %q, want next shared auth %q", gotKey1Again.ID, "auth-c")
	}
}
