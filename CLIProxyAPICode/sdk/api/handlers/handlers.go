...
func requestExecutionMetadata(ctx context.Context) map[string]any {
	...
	if apiKeyHash, apiKeyIndex := downstreamAPIKeyMetadataFromContext(ctx); apiKeyHash != "" {
		meta[coreexecutor.DownstreamAPIKeyHashMetadataKey] = apiKeyHash
		if apiKeyIndex != "" {
			meta[coreexecutor.DownstreamAPIKeyIndexMetadataKey] = apiKeyIndex
		}
	}
...
func downstreamAPIKeyMetadataFromContext(ctx context.Context) (string, string) {
	if ctx == nil {
		return "", ""
	}
	...
	apiKey := handlerStringValue(rawAPIKey)
	if apiKey == "" {
		return "", ""
	}
	return hashDownstreamAPIKey(apiKey), downstreamAPIKeyIndexFromGin(ginCtx)
}

func downstreamAPIKeyIndexFromGin(ginCtx *gin.Context) string {
	if ginCtx == nil {
		return ""
	}
	...
	case map[string]string:
		return strings.TrimSpace(metadata["index"])
	case map[string]any:
		return handlerStringValue(metadata["index"])
	default:
		return ""
	}
}

func hashDownstreamAPIKey(apiKey string) string {
...
func handlerStringValue(raw any) string {
...
func (h *BaseAPIHandler) GetContextWithCancel(handler interfaces.APIHandler, c *gin.Context, ctx context.Context) (context.Context, APIHandlerCancelFunc) {
	...
	// 正文捕获对象由 HTTP 中间件创建。即使调用方传入 context.Background()，
	// 这里也要从真实请求 context 显式继承，保证 usage 插件能等待最终响应完成后再落盘。
	if contentCapture := coreusage.ContentCaptureFromContext(requestCtx); contentCapture != nil {
		newCtx = coreusage.WithContentCapture(newCtx, contentCapture)
	}
