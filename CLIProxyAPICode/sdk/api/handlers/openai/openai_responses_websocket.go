...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		requestNeedsPinnedAuth := responsesWebsocketRequestNeedsPinnedAuth(payload)
		if !requestNeedsPinnedAuth {
			pinnedAuthID = ""
		} else if pinnedAuthID == "" && lastSuccessfulAuthID != "" {
			pinnedAuthID = lastSuccessfulAuthID
		}
		requestPinnedAuthID := pinnedAuthID
		lastAttemptedAuthID = ""
		cliCtx, cliCancel := h.GetContextWithCancel(h, c, context.Background())
		// WebSocket 请求正文位于每一条 response.create/response.append 消息中，
		// 不经过普通 HTTP Body 中间件，因此在每轮执行前单独建立正文捕获对象。
		contentCapture := coreusage.NewContentCapture(requestJSON)
		cliCtx = coreusage.WithContentCapture(cliCtx, contentCapture)
		...
		if requestPinnedAuthID != "" {
			...
			cliCtx = handlers.WithSelectedAuthIDCallback(cliCtx, func(authID string) {
				...
				if requestNeedsPinnedAuth && websocketUpstreamSupportsIncrementalInput(selectedAuth.Attributes, selectedAuth.Metadata) {
					pinnedAuthID = authID
				}
			})
		}
		...
		if errForward != nil {
			contentCapture.Complete([]byte(errForward.Error()))
			wsTerminateErr = errForward
			log.Warnf("responses websocket: forward failed id=%s error=%v", passthroughSessionID, errForward)
			return
		}
		if forwardErrMsg != nil && forwardErrMsg.Error != nil {
			contentCapture.Complete([]byte(forwardErrMsg.Error.Error()))
		} else {
			contentCapture.Complete(completedOutput)
		}
		if forwardErrMsg == nil && lastAttemptedAuthID != "" {
			lastSuccessfulAuthID = lastAttemptedAuthID
		}
		if forwardErrMsg == nil && requestNeedsPinnedAuth && !useUpstreamWebsocketPassthrough && lastAttemptedAuthID != "" {
...
func responsesWebsocketRequestNeedsPinnedAuth(payload []byte) bool {
	if len(payload) == 0 {
		return false
	}
	...
	return strings.TrimSpace(gjson.GetBytes(payload, "type").String()) == wsRequestTypeAppend
}
